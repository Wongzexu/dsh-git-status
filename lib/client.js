...
const I18N = {
  zh: {
    ...
    gitSwitchLocalExistsText: '本地已存在同名分支 {branch}（{remote}）。检出到已有分支，或换个名字从远程创建？',
    gitSwitchLocalExistsBtn: '检出已有分支',
    gitSwitchLocalExistsNewBtn: '其他名称',
    gitCreateFromRemoteTitle: '创建本地分支并检出（{remote}）',
    gitCreateFromRemotePrompt: '请输入新分支名（检出自 {remote}）',
  ...
  en: {
    ...
    gitSwitchLocalExistsText: 'A local branch named {branch} already exists ({remote}). Check out the existing branch, or create a local branch with a different name?',
    gitSwitchLocalExistsBtn: 'Check out the existing branch',
    gitSwitchLocalExistsNewBtn: 'Create with a different name',
    gitCreateFromRemoteTitle: 'Create local branch & check out ({remote})',
    gitCreateFromRemotePrompt: 'Enter a new branch name (checked out from {remote})',
...
// ---------- Git Graph（右缘浮窗，移植 vscode-git-graph 泳道算法） ----------
     ...
     *  检出已有分支 / 换个名字从远程创建 / 取消（对齐上游 checkoutBranchAction）。 */
      ...
      try {
        ...
        if (err.code === 'branch-already-exists' && payload.remote !== '') {
          gitConfirmOpen({
            ...
            onOk: () => gitCheckout({ ...payload, remote: '' }),
    ...
    const gitCreateRun = async (force = false) => {
      ...
      try {
        if (gitCreateMode === 'rename') {
          ...
          // 换名从远程创建本地跟踪分支并检出（服务端权威校验：重名/不存在/守卫）。
          const result = await gitBranchAction({ action: 'checkout', branch: name, remote: gitCreateRemote, force })
          gitCreateClose()
    ...
    const gitDisplaySection = () => {
      ...
      // 长日志选项 2 个一行：包含 reflog 提交 / 只跟随第一个父提交
      ...
      rowLog.appendChild(gitDisplayRow('onlyFirstParent', 'gitDisplayFirstParent', 'gitDisplayFirstParentHint', true))
      wrap.appendChild(rowLog)
      // 长文案「徽标合并」放最后一行：与长行同宽左对齐
    ...
    const gitDefaultSection = () => {
      const box = document.createElement('div')
      box.appendChild(gitDefaultPruneRow())
