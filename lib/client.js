...
const I18N = {
  zh: {
    ...
    gitDeleteOkSync: '已删除分支 {branch}（同步删除远程 {remotes} 的同名分支）',
    gitDeleteSyncNotice: '已开启「删除本地分支时同步删除远程」：若远程存在同名分支将一并删除（git push --delete）。',
    gitDeleteSyncFailed: '本地分支已删除；远程删除失败：{detail}',
    gitDeleteBtn: '删除',
    ...
    gitDefaultSyncDeleteRemote: '删除本地分支时同步删除远程',
    gitDefaultSyncDeleteRemoteHint: '删除本地分支（含强制删除）时，若远程存在同名分支则一并删除（git push --delete）；无同名远程分支时不受影响',
  ...
  en: {
    ...
    gitDeleteOkSync: 'Branch {branch} deleted (remote {remotes} deleted too)',
    gitDeleteSyncNotice: '"Sync-delete remote" is enabled: a same-named remote branch will be deleted too (git push --delete).',
    gitDeleteSyncFailed: 'Local branch deleted; remote deletion failed: {detail}',
    gitDeleteBtn: 'Delete',
    ...
    gitDefaultSyncDeleteRemote: 'Delete remote branch when deleting local',
    gitDefaultSyncDeleteRemoteHint: 'When deleting a local branch (incl. force-delete), also delete a same-named remote branch (git push --delete); no effect if no such remote branch exists',
...
// ---------- Git Graph（右缘浮窗，移植 vscode-git-graph 泳道算法） ----------
        ...
        gitCtxOpen(ev.clientX, ev.clientY, [
          ...
          },
          {
            ...
            onClick: () => gitConfirmOpen({
              ...
              text: gitDeleteConfirmText(branchName),
              okText: t('gitDeleteBtn'),
              danger: true,
              onOk: async () => {
                try {
                  const res = await gitBranchAction({ action: 'delete', branch: branchName, syncRemote: gitDeleteSyncOn() })
                  gitDeleteFlash(res)
                  gitFetch(true, true)
                } catch (err) {
                  flash(gitErrText(err), 'error')
          ...
          },
          {
            ...
            onClick: () => gitConfirmOpen({
              ...
              text: gitDeleteConfirmText(branchName, true),
              okText: t('gitDeleteForceBtn'),
              danger: true,
              onOk: async () => {
                try {
                  const res = await gitBranchAction({ action: 'delete', branch: branchName, force: true, syncRemote: gitDeleteSyncOn() })
                  gitDeleteFlash(res)
                  gitFetch(true, true)
                } catch (err) {
                  flash(gitErrText(err), 'error')
    ...
    // 删除本地分支的确认/结果文案：默认行为「删除本地分支时同步删除远程」开启时，
    // 确认框追加提示；结果 flash 区分「同步删成功 / 本地已删但远程失败 / 仅删本地」。
    const gitDeleteSyncOn = () => gitSettingsStoreLoad().syncDeleteRemote === true
    const gitDeleteConfirmText = (branchName, force = false) => {
      const base = t(force ? 'gitDeleteForceConfirm' : 'gitDeleteConfirm', { branch: branchName })
      return gitDeleteSyncOn() ? `${base}\n${t('gitDeleteSyncNotice')}` : base
    }
    const gitDeleteFlash = (res) => {
      const failed = Array.isArray(res.failed) && res.failed.length > 0 ? res.failed : null
      const synced = Array.isArray(res.synced) && res.synced.length > 0 ? res.synced : null
      if (failed !== null) {
        flash(t('gitDeleteSyncFailed', { detail: failed.join('；') }), 'error')
        return
      }
      if (synced !== null) {
        flash(t('gitDeleteOkSync', { branch: res.branch ?? '', remotes: synced.join(', ') }))
        return
      }
      flash(t('gitDeleteOk', { branch: res.branch ?? '' }))
    }
    ...
    const gitDefaultSection = () => {
      ...
      box.appendChild(gitDefaultBooleanRow('syncDeleteRemote', 'gitDefaultSyncDeleteRemote', 'gitDefaultSyncDeleteRemoteHint'))
