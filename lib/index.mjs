 ...
 */
async function gitBranchAction(root, action, { branch = '', remote = '', name = '', force = false, start = '', noff = false } = {}) {
  ...
  if (action === 'merge') {
    ...
    if (blocked !== null) return { ok: false, error: blocked }
    try {
      // noff（NoFF）：可快进时也强制生成合并提交；argv 数组无 shell，参数固定无注入面。
 ...
 *  返回按名排序的 [{ name, url, pushUrl }]；pushUrl 无 → null（UI 显示「同 fetch URL」）。
 *  多值取最后一条（parseConfigList 语义；UI 单值模型，编辑保存时 set-url 整体覆盖）。 */
async function gitRemoteConfig(root) {
  const map = await gitConfigList(root, 'local')
  const byName = new Map()
  for (const [key, value] of map) {
    const m = /^remote\.(.+)\.(url|pushurl)$/.exec(key)
    if (m === null) continue
    let entry = byName.get(m[1])
    if (entry === undefined) {
      entry = { name: m[1], url: null, pushUrl: null }
      byName.set(m[1], entry)
    }
    if (value !== '') {
      if (m[2] === 'url') entry.url = value
...
export function registerRoutes(ctx, { events = {} } = {}) {
  ...
  //   branch, remote, name, force, start, noff, session }。
