...
/**
 * 推送分支（写路由，镜像上游 dataSource.pushBranch）：
 * `git push <remote> <branch> [--set-upstream] [--force-with-lease|--force]`。
 * - 分支名 validateBranchName + 本地存在性；remote 名白名单 + 存在性权威校验（同 fetch）
 * - setUpstream 布尔；mode 白名单枚举（argv 无 shell，参数全白名单）
 * - 超时放宽：GIT_FETCH_TIMEOUT（慢网络大仓库）
 * 返回 { ok: true } 或 { ok: false, error: { code, message } }。
 */
async function gitPushAction(root, { branch = '', remote = '', setUpstream = false, mode = 'normal' } = {}) {
  const error = (code, message) => ({ ok: false, error: { code, message } })
...
export function registerRoutes(ctx, { events = {} } = {}) {
  ...
  // { branch, remote, setUpstream: bool, mode: 'normal'|'force-with-lease'|'force', session }。
  // CSRF 防护同 branch 路由（强制 application/json）。
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    path: GIT_PUSH_PATH,
