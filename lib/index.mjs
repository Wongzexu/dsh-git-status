...
// 自 dsh-companion 抽离独立（守卫模型移植自社区 dsh-git-graph）。
//
...
async function gitLogV2(root, opts = {}) {
  const [head, stashes, uncommitted, remoteRefs] = await Promise.all([
    gitHead(root),
    gitStashes(root),
  ...
  if (logResult.logError !== undefined) {
    ...
    return { commits: [], moreAvailable: false, head, uncommitted }
  }
  ...
  return { commits: rows, moreAvailable, head, uncommitted }
}
...
async function gitShow(root, rev, base = '') {
  ...
  const metaOut = await runGit(root, ['show', '-s', `--format=${metaFmt}`, rev])
...
/**
 * 未提交改动详情（移植上游 getUncommittedDetails）：
 * diff HEAD（staged+unstaged）为文件/patch，`status -s --porcelain -z` 补未跟踪/已删除。
 */
async function gitShowUncommitted(root) {
  const [numstatOut, statusOut, patchOut] = await Promise.all([
    runGit(root, ['diff', 'HEAD', '--numstat']).catch(() => ''),
    runGit(root, ['status', '-s', '--untracked-files=all', '--porcelain', '-z']).catch(() => ''),
    runGit(root, ['diff', 'HEAD', '--no-color']).catch(() => ''),
  ])
  ...
  for (const entry of statusOut.split('\0')) {
    ...
    if (path === '' || (!code.includes('?') && !code.includes('!') && !code.includes('D'))) continue
    if (!files.some((f) => f.path === path)) files.push({ path, adds: 0, dels: 0, status: code })
  }
  let patch = patchOut
  let truncated = false
  if (patch.length > PATCH_MAX) {
...
function extractBlockedPaths(stderr, header) {
  ...
  for (const line of stderr.slice(start + 1).split('\n')) {
    ...
    paths.push(quoted === null ? trimmed.replace(/\\(.)/g, '$1') : (quoted[1] ?? '').replace(/\\(.)/g, '$1'))
  }
...
async function gitGuardBlock(root, target) {
  try {
    const conflicts = await runGit(root, ['diff', '--name-only', '--diff-filter=U'])
    const count = conflicts.split(/\r?\n/).filter((line) => line !== '').length
 ...
 */
async function gitBranchAction(root, action, { branch = '', remote = '', name = '' } = {}) {
  ...
  if (action === 'create') {
    ...
    if (blocked !== null) return { ok: false, error: blocked }
    try {
      await runGit(root, ['switch', '--no-guess', '-c', name])
...
export function apply(ctx) {
  ctx.effect(() => {
    ...
    // 分支操作（写路由）：POST JSON body { action: 'checkout'|'create', branch, remote, name, session }。
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          if (action !== 'checkout' && action !== 'create') return json(res, 400, { error: 'unknown action' })
  ...
  }, 'dsh-git-status: git log/show/branch routes')
}
