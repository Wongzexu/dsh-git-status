...
export const GIT_HISTORY_PATH = '/plugins/dsh-gitstatus/git/history'
...
/**
 * Rebase 目标解析（权威校验，客户端镜像不信任）：hex hash（7-40 位）按 commit
 * 存在性校验（`^{commit}` 剥壳）；其余按本地分支名校验 + refs/heads 存在性。
 * 返回 { ok:true, value } 或 { ok:false, error }。
 */
async function resolveRebaseTarget(root, target) {
  if (typeof target !== 'string' || target === '') {
    return { ok: false, error: { code: 'invalid-target', message: 'invalid target' } }
  }
  if (/^[0-9a-f]{7,40}$/i.test(target)) {
    try {
      await runGit(root, ['rev-parse', '--verify', '--quiet', `${target}^{commit}`])
      return { ok: true, value: target }
    } catch {
      return { ok: false, error: { code: 'target-not-found', message: `commit "${target}" does not exist` } }
    }
  }
  if (validateBranchName(target) !== null) {
    return { ok: false, error: { code: 'invalid-branch-name', message: 'invalid branch name' } }
  }
  if (!(await gitRefExists(root, `refs/heads/${target}`))) {
    return { ok: false, error: { code: 'target-branch-not-found', message: `branch "${target}" does not exist` } }
  }
  return { ok: true, value: target }
}

/** commit hash 权威校验（hex 7-40 位 + `^{commit}` 存在性），返回全 hash。 */
async function resolveCommitHash(root, hash) {
  if (typeof hash !== 'string' || !/^[0-9a-f]{7,40}$/i.test(hash)) {
    return { ok: false, error: { code: 'invalid-target', message: 'invalid target commit' } }
  }
  try {
    const out = await runGit(root, ['rev-parse', '--verify', '--quiet', `${hash}^{commit}`])
    return { ok: true, value: out.trim() }
  } catch {
    return { ok: false, error: { code: 'target-not-found', message: `commit "${hash}" does not exist` } }
  }
}

/** 目标 commit 的父提交列表：`git rev-list --parents -n 1 <hash>` 首行第二列起
 *  （第一列为自身）；失败返回 []（不可达时上层已先校验存在性）。 */
async function gitCommitParents(root, hash) {
  try {
    const line = (await runGit(root, ['rev-list', '--parents', '-n', '1', hash])).trim()
    return line.split(/\s+/).filter((p) => p !== '').slice(1)
  } catch {
    return []
  }
}

/** a 是否为 b 的祖先（`git merge-base --is-ancestor` 退出码：0=是，1=否）。 */
async function isAncestor(root, a, b) {
  try {
    await runGit(root, ['merge-base', '--is-ancestor', a, b])
    return true
  } catch {
    return false
  }
...
/**
 * rebase/cherry-pick/revert/reset 失败 → 稳定错误码（stdout/stderr 两流都查，
 * 冲突叙述可能在 stdout，如 merge）。kind 决定冲突码后缀（rebase-conflicts 等）。
 * - 冲突 / cherry-pick「now empty」→ <kind>-conflicts
 * - 工作区改动被挡（would be overwritten by cherry-pick/revert/…）→ tracked-changes-would-be-overwritten
 * - rebase 未暂存改动/索引改动拒绝 → rebase-uncommitted
 * - 游离 HEAD 做 rebase → not-on-branch
 * - 并发已有进行中操作（理论上被前置守卫拦住）→ operation-in-progress
 * 其余 internal（客户端回显首行原文）。
 */
function classifyHistoryFailure(message, stdout, kind) {
  const text = `${message}\n${stdout}`
  const head = (text.split('\n').find((line) => line.trim() !== '') ?? text).trim()
  if (/CONFLICT|conflict|now empty/i.test(text)) {
    return { code: `${kind}-conflicts`, message: head }
  }
  if (/would be overwritten by (cherry.?pick|revert|rebase|merge|checkout)/i.test(text)) {
    return { code: 'tracked-changes-would-be-overwritten', message: head }
  }
  if (/cannot rebase: You have (unstaged|uncommitted) changes|cannot rebase: Your index contains uncommitted changes/i.test(text)) {
    return { code: 'rebase-uncommitted', message: head }
  }
  if (/not currently on a branch/i.test(text)) {
    return { code: 'not-on-branch', message: head }
  }
  if (/already (?:in progress|exists)|already a .+ in progress/i.test(text)) {
    return { code: 'operation-in-progress', message: head }
  }
  return { code: 'internal', message: head || `git ${kind} failed` }
}

/** reset 模式白名单（镜像上游 GitResetMode）。 */
const RESET_MODES = ['soft', 'mixed', 'hard']

/**
 * 历史破坏性操作（写路由，镜像上游 dataSource 的 rebase/reset/cherry-pick/revert/pull）：
 * - rebase：`git rebase <branch|hash>`（仅非交互——交互终端 web 环境不可行）；
 *   rebase-merge/rebase-apply 标记已在 OPERATION_MARKERS，头部徽标自动显示进行中。
 * - reset：`git reset --<soft|mixed|hard> <hash>`；祖先守卫——目标非当前分支祖先
 *   （分支独有提交将被丢弃）且未带 force → reset-not-ancestor 拒绝，
 *   客户端确认后带 force 重发（同切换守卫 force 旁路模式）。
 * - cherry-pick：`git cherry-pick [-x] [--no-commit] [-m n] <hash>`；
 *   merge commit 强制 parent（1-based 白名单，超出拒绝）。
 * - revert：`git revert --no-edit [-m n] <hash>`；merge commit 强制 parent。
 * - pull：`git pull <remote> <branch> [--squash|--no-ff]`；remote 名存在性 + 分支名
 *   白名单，冲突归现有 merge 分类（MERGE_HEAD 或 squash 的 SQUASH_MSG 由合并条接管）。
 * 所有操作前置进行中操作/冲突守卫（gitGuardBlock），失败分类见 classifyHistoryFailure。
 */
async function gitHistoryAction(root, action, { target = '', hash = '', branch = '', remote = '', mode = 'mixed', parent = 0, noff = false, squash = false, force = false, recordOrigin = false, noCommit = false } = {}) {
  const error = (code, message) => ({ ok: false, error: { code, message } })
  const guard = await gitGuardBlock(root, undefined)
  if (guard !== null) return { ok: false, error: guard }

  if (action === 'rebase') {
    const obj = await resolveRebaseTarget(root, target)
    if (!obj.ok) return { ok: false, error: obj.error }
    try {
      await runGit(root, ['rebase', obj.value])
      return { ok: true }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err)
      const stdout = err instanceof Error && typeof err.stdout === 'string' ? err.stdout : ''
      return { ok: false, error: classifyHistoryFailure(msg, stdout, 'rebase') }
    }
  }
  if (action === 'reset') {
    if (!RESET_MODES.includes(mode)) return error('invalid-reset-mode', 'invalid reset mode')
    const obj = await resolveCommitHash(root, hash)
    if (!obj.ok) return { ok: false, error: obj.error }
    if (!(await isAncestor(root, obj.value, 'HEAD')) && force !== true) {
      // 非祖先：重置会把分支移到无关 commit，当前分支独有提交全部失去（仅 reflog 可救）。
      // 由客户端确认后带 force 重发；错误消息留给客户端本地化（不给 message 防中英混杂）。
      return error('reset-not-ancestor', '')
    }
    try {
      await runGit(root, ['reset', `--${mode}`, obj.value])
      return { ok: true, hash: (await runGit(root, ['rev-parse', '--short', 'HEAD'])).trim() }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err)
      return { ok: false, error: classifyHistoryFailure(msg, '', 'reset') }
    }
  }
  if (action === 'cherry-pick') {
    const obj = await resolveCommitHash(root, hash)
    if (!obj.ok) return { ok: false, error: obj.error }
    const parents = await gitCommitParents(root, obj.value)
    const args = ['cherry-pick']
    if (recordOrigin === true) args.push('-x')
    if (noCommit === true) args.push('--no-commit')
    if (parents.length > 1) {
      if (!Number.isInteger(parent) || parent < 1 || parent > parents.length) {
        return error('invalid-mainline', `merge commit requires a valid mainline parent (1-${parents.length})`)
      }
      args.push('-m', String(parent))
    }
    args.push(obj.value)
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err)
      const stdout = err instanceof Error && typeof err.stdout === 'string' ? err.stdout : ''
      return { ok: false, error: classifyHistoryFailure(msg, stdout, 'cherry-pick') }
    }
  }
  if (action === 'revert') {
    const obj = await resolveCommitHash(root, hash)
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err)
      const stdout = err instanceof Error && typeof err.stdout === 'string' ? err.stdout : ''
      return { ok: false, error: classifyHistoryFailure(msg, stdout, 'revert') }
    }
  }
  if (action === 'pull') {
    ...
    if (validateBranchName(branch) !== null) return error('invalid-branch-name', 'invalid branch name')
    // --no-rebase 强制合并语义：不依赖用户全局 pull.rebase 配置（否则分叉时
    // git 2.27+ 直接报「Need to specify how to reconcile divergent branches」）。
    const args = ['pull', '--no-rebase']
    if (squash === true) args.push('--squash')
    else if (noff === true) args.push('--no-ff')
    ...
    try {
      ...
      const classified = classifyFetchFailure(msg)
      if (classified.code !== 'internal') return { ok: false, error: classified }
      return { ok: false, error: classifyHistoryFailure(msg, stdout, 'pull') }
    }
  }
 ...
 */
async function gitBranchAction(root, action, { branch = '', remote = '', name = '', force = false, start = '', noff = false, squash = false, message = '', fastForward = '', checkout = true, syncRemote = false } = {}) {
  const error = (code, message) => ({ ok: false, error: { code, message } })
  if (action === 'checkout') {
  ...
  if (action === 'delete') {
    ...
    // syncRemote（默认行为设置「删除本地分支时同步删除远程」）：本地删除成功后，
    // 对每个存在同名远程跟踪 ref 的远程执行 git push --delete，与 gitRemoteAction
    // 的 delete-branch 同语义；远程分支已不存在时降级只删本地跟踪 ref（branch -d -r）。
    // 失败逐个收集上报：本地删除已成功，不整体失败（客户端提示「本地已删，远程失败」）。
    const result = { ok: true, branch }
    if (syncRemote === true) {
      const synced = []
      const failed = []
      for (const remote of await gitRemoteList(root)) {
        if (!(await gitRefExists(root, `refs/remotes/${remote}/${branch}`))) continue
        try {
          ...
          if (/remote ref does not exist/.test(msg)) {
            try {
              ...
              failed.push(`${remote}: ${(msg.split('\n')[0] ?? msg).trim()}`)
            }
          } else {
            failed.push(`${remote}: ${classifyPushFailure(msg).message}`)
          }
        }
      }
...
export function registerRoutes(ctx, { events = {} } = {}) {
    ...
    handler: async (req, res) => {
      try {
        ...
        const result = await gitBranchAction(root, action, {
          ...
          syncRemote: payload.syncRemote === true,
        })
        json(res, 200, result)
      } catch (error) {
  ...
  // 历史操作（写路由）：POST JSON body
  // { action: 'rebase'|'reset'|'cherry-pick'|'revert'|'pull', target, hash, branch,
  //   remote, mode, parent, noff, squash, force, recordOrigin, noCommit, session }。
  // CSRF 防护同 branch 路由（强制 application/json content-type）。
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    ...
    handler: async (req, res) => {
      try {
        ...
        if (typeof payload !== 'object' || payload === null) return json(res, 400, { error: 'malformed body' })
        const action = payload.action
        const ACTIONS = ['rebase', 'reset', 'cherry-pick', 'revert', 'pull']
        if (!ACTIONS.includes(action)) return json(res, 400, { error: 'unknown action' })
        const workspace = resolveWorkspace(ctx, payload.session ?? '')
        if (!workspace.ok) return workspaceError(res, workspace, true)
        const root = workspace.root
        if (!(await gitIsRepo(root))) return json(res, 200, { ok: false, error: { code: 'internal', message: 'not a git repository' } })
        const result = await gitHistoryAction(root, action, {
          target: typeof payload.target === 'string' ? payload.target : '',
          hash: typeof payload.hash === 'string' ? payload.hash : '',
          branch: typeof payload.branch === 'string' ? payload.branch : '',
          remote: typeof payload.remote === 'string' ? payload.remote : '',
          mode: typeof payload.mode === 'string' ? payload.mode : 'mixed',
          parent: Number.isInteger(payload.parent) ? payload.parent : 0,
          noff: payload.noff === true,
          squash: payload.squash === true,
          force: payload.force === true,
          recordOrigin: payload.recordOrigin === true,
          noCommit: payload.noCommit === true,
        })
        json(res, 200, result)
      } catch (error) {
        json(res, 500, { ok: false, error: { code: 'internal', message: error instanceof Error ? error.message : String(error) } })
